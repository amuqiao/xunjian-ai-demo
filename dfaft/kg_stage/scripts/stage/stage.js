 ...
 *   3. L7 顶台三枚徽章（技术攻关/平台建设/人才发展）当前没有对应数据节点，做成
 ...
 *   5. 左侧六边形入口（文档搜索/图谱搜索/图谱介绍）的差异化落点在 core/router.js
...
(function () {
  ...
  function showStats() {
    ...
    hintTypeEl.textContent = '图谱总览';
    hintNameEl.textContent = '';
    hintDescEl.textContent = st.nodes + ' 节点 / ' + st.edges + ' 关系 / ' + st.docs + ' 文档';
  }
   ...
   *   tech-attack（技术攻关）—— view=task    十大重点任务本身就是"技术攻关"的载体
   *   platform    （平台建设）—— view=business 平台建设服务于三大业务，语义最贴近
   *   talent      （人才发展）—— view=domain   人才培养围绕七大技术领域布局
  ...
  function bindBadges() {
    ...
    dom.qsa('.badge', pageEl).forEach(function (badge) {
      var label = badge.textContent || '技术攻关';
