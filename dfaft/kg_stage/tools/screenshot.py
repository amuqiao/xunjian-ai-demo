...
def check_basic_load(browser, url, label, id_prefix):
    ...
    try:
        ...
        title = ''
    record(f'{id_prefix}1', f'[{label}] page.title() 精确匹配', title == '国家管网技术图谱', f'实测: {title!r}')
...
def check_navigation_flows(page):
    ...
    # 18. 六边形 3 个入口落点
    goto_and_settle_stage(page)
    page.click('.hex[aria-label="文档搜索"]')
    ...
    record('18', '六边形三入口（文档搜索/图谱搜索/图谱介绍）落点正确',
           ok18a and ok18b and ok18c,
           f'文档搜索: {h18a} | 图谱搜索: {h18b} | 图谱介绍: {h18c}')
...
def check_legend_colors(page):
    ...
    出现，图谱图例天然没有"攻关内容"一项——这不是 bug，图例四色断言里"图谱页"
    只校验三色，"文档页"才校验四色。"""
    page.evaluate("() => { location.hash = '#/graph?view=task'; }")
    wait_js(page, "() => location.hash.indexOf('view=task') !== -1")
    page.wait_for_timeout(300)
    ...
    """)
    type_labels_graph = {'task': '重点任务', 'direction': '攻关方向', 'technology': '攻关技术'}
    type_labels_docs = {'task': '重点任务', 'direction': '攻关方向', 'technology': '攻关技术', 'content': '攻关内容'}
