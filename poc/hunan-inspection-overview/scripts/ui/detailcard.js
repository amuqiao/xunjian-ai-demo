...
//
// activeZoneLabel 是 3D 契约钩子：契约是——任何承载 3D 的场景，其“当前作业区”显示元素
// 必须带 HunanContract.ACTIVE_LABEL_ATTR（data-active-zone-label），文本内容为该作业区
// 的中文名。这里直接用契约常量当属性 key（属性值固定为 "1"，只是“这是那个元素”的
// 存在性标记，真正要给验证脚本读的可见文本走 text:），不在本文件里重新硬编码那个
// 属性名字符串。scripts/map3d/ 下的场景负责在渲染时把当前作业区的中文名传进
// activeZoneLabel。
(function () {
  "use strict";

  var C = window.HunanContract;
  ...
  function render(options) {
    ...
    if (options.activeZoneLabel != null) {
      assertNonEmptyString(options.activeZoneLabel, "DetailCard 的 activeZoneLabel");
      var labelAttrs = { class: "detail-active-part", text: options.activeZoneLabel };
