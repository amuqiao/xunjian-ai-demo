...
def run(page):
    ...
    check("工作台记录表新增到 7 条 P-1 表单项", page.locator(".sl-table-row").count() >= 7)
    ...
    check("新增 2号轴承 RAG 表单项可见", "2号轴承振动值异常上升" in text_of(page, ".wb-records"))
    ...
    check("点击 2号轴承表单项打开 AI 辅助判断列表浮层", page.locator(".wb-ai-list-overlay").count() == 1)
    check("2号轴承浮层包含内圈剥落建议", "疑似轴承内圈剥落" in text_of(page, ".wb-ai-list-overlay"))
    check("2号轴承浮层展示 87% 置信度", "87%" in text_of(page, ".wb-ai-list-overlay"))
    check("2号轴承浮层展示处置建议匹配度 65%", "65%" in text_of(page, ".wb-ai-list-overlay"))
    check("2号轴承浮层包含 72h 停机检查建议", "72h" in text_of(page, ".wb-ai-list-overlay"))
    page.click('.wb-ai-list-overlay [data-action="open-evidence"][data-evidence-kind="series"]')
    page.wait_for_selector(".dt-chart canvas", timeout=3000)
    check("2号轴承表单项可进入告警数据详情", "2号轴承振动告警" in text_of(page, ".dt-main"))
    page.click('[data-action="close-detail"]')
    page.locator(".sl-table-row").filter(has_text="2号轴承振动值异常上升").click()
    page.click('.wb-ai-list-overlay [data-action="open-evidence"][data-evidence-kind="vision"]')
    check("2号轴承表单项可进入视觉排除性证据", "排除性证据" in text_of(page, ".scene-shell"))
    check("2号轴承视觉详情包含无渗漏和无明显偏移", "无渗漏" in text_of(page, ".scene-shell") and "无明显偏移" in text_of(page, ".scene-shell"))
    page.click('[data-action="close-detail"]')
    page.locator(".sl-table-row").filter(has_text="2号轴承振动值异常上升").click()
    ...
    reject_outcome = page.evaluate("() => window.DOMAIN_REVIEW.outcomes.filter(o => o.id === 'reject')[0].id")
    check("反查到驳回结论且与 AI 建议不同", reject_outcome != suggested)
    check("驳回自动选择排除误报结论", page.evaluate("() => window.AppState.value.review.outcomeId") == reject_outcome)
