...
// 本文件与 poc/hunan-inspection-overview/scripts/boot.js 结构逐字节相同（两级钻取的状态
...
(function () {
  ...
  function mountMap3D() {
    ...
    // 约束：姐妹 POC（hunan-inspection-overview）实测点开站点最多的作业区（岳阳，
    // 36 站）时 renderCalls 已经到 192（engine.js 的热点池按可见热点数 × 固定
    // 开销，站点越多开销越高，这部分不能从本项目这边省），管道 Tube 网格再叠上去
    // 会击穿 <200 护栏。省域态余量充足，本 POC 只有 3 条成品油管道，代价更低。
