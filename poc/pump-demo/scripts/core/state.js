...
(function () {
  ...
  // 半坏状态是最容易在现场翻车的东西。
    ...
    var STORAGE_KEY = "pump-demo-v10-state";
  ...
  function canOpenForState(sceneKey, target) {
    if (sceneKey === "overview" || sceneKey === "station" || sceneKey === "workbench") return true;
    if (sceneKey === "confirm") return target.diagnosisReady || target.expertVerdict !== "" || target.treatmentDone || target.observationDone || target.archived;
    if (sceneKey === "archive") return target.treatmentDone || target.observationDone || target.archived;
    if (sceneKey === "knowledge" || sceneKey === "graph") return true;
    return false;
  }
