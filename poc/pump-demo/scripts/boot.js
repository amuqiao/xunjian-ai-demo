...
(function () {
  ...
  function renderScene() {
    if (state.scene === "overview") return Scenes.renderOverview();
    if (state.scene === "station") return Scenes.renderStation();
    // 时序/视觉详情是 workbench 的**场景内子屏**，不是第 8/9 个场景：
    //   1. 场景导航的 01-07 编号本身就是演示叙事，加两个编号会破坏它，还要再引入
    //      第三套锁语义（"只能从 workbench 进入"）；
    //   2. 底部 8 步流程条里 trend(第 4 步) / vision(第 5 步) 早就被定义成路由到
    //      workbench 的步骤，现有心智模型就是"它们在部位诊断之内"；
    //   3. 代码里已有先例——confirm 靠 state.closureOpen 切到处置执行屏（下一行），
    //      用同一个模式，不发明第二套。
    // 视觉上占满整个 stage（不在网格里塞展开态），但场景导航仍高亮"诊断工作台"。
    if (state.scene === "workbench" && state.detail) return Scenes.renderDetailScreen();
    if (state.scene === "workbench") return Scenes.renderWorkbench();
    if (state.scene === "confirm") return state.closureOpen ? Scenes.renderTreatment() : Scenes.renderConfirm();
    if (state.scene === "archive") return Scenes.renderArchive();
    return Scenes.renderGraph();
  }
  ...
  function renderSceneCharts() {
    ...
    // workbench 有两套互斥的视图，各自有自己的槽位，必须按 state.detail 二选一：
    // state.detail 非空时整屏切到时序/视觉详情（scenes/detailscreen.js，见上面
    // renderScene() 的同一段注释）；为空时是 workbench 自身，它的「时序模型」卡在
    // 阶段三整合时从手写 SVG（已删除的 AppCharts.miniChart）换成了
    // Charts.slot + ChartOptions.spark，所以同样需要这个钩子。
    // 与 overview 分支同一种分工：只负责 draw()，不负责创建/挂载槽位容器
    // （那一步是下面的 mountChartSlots()）。
    if (state.scene === "workbench") {
      if (state.detail) Scenes.renderDetailScreenCharts();
      else Scenes.renderWorkbenchCharts();
    }
    // station 也有真正的 ECharts（部位风险雷达 + 机组对照的 4 条 spark）。漏掉这个分支的
    // 后果是"容器挂上了但永远没人往里 draw"——页面上是一块**纯黑的空卡**，而且不报错、
    // 不影响任何断言（mountChartSlots 只保证容器就位，Charts.flush 没有待绘队列就什么
    // 都不做）。这一整类"图表静默空白"只能靠看截图发现，所以每次新增使用 Charts 的
    // 场景，都必须同步在这里加分支。
    if (state.scene === "station") Scenes.renderStationCharts();
    if (state.scene === "graph") Scenes.renderGraphCharts();
  }
