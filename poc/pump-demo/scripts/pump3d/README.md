...
```bash
# 3D 全量验收（全部断言 + 三档宽度截图；历史基线 44 项全绿）
uv run python poc/pump-demo/verify/verify_pump3d.py

# CSS 计算样式基线比对（专治「肉眼看不出但 computedStyle 变了」）
uv run python poc/pump-demo/verify/verify_css_snapshot.py
# 仅在逐条 review 过差异后才更新基线：
uv run python poc/pump-demo/verify/verify_css_snapshot.py --write
```
