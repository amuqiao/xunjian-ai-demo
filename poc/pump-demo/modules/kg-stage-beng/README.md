...
这是从 `demo/cc/kg_stage` 复制出的 BENG 模板落地版。页面仍然是纯静态大屏：无构建、无框架、无 CDN、无网络请求，双击 `index.html` 即可运行。
...
```bash
open demo/cc/kg_stage_beng/index.html
```

也可以用本地 HTTP 服务复测：

```bash
python3 -m http.server 8080 --directory demo/cc/kg_stage_beng
```

打开 `http://localhost:8080/#/stage`。
...
| `tools/screenshot.py` | Playwright 页面回归检查脚本 |
