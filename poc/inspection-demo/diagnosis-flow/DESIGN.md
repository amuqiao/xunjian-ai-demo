...
```text
③ 诊断工作台 ──→ ④ 人工复核 ──→ ⑤ 报告归档 ──→ ⑥ 知识库
  报表 + AI 判断     表决/结论/       报告草稿         文档 +1
  ├─ 时序子屏        结构化/意见       ├─ 人工原文回显    ├─ 入库动画
  ├─ 视觉子屏        ├─ 分歧必填理由    └─ 归档飞入 ───→  └─ Agent 可命中
  └─ Agent 入口      └─ 复测不通过 ↺                          │
                        退回复核                              │
      ↑                                                       │
...
**在范围内**：诊断工作台、时序详情、视觉详情、Agent 对话、人工复核、报告归档、知识库。
...
4 个主页面 + 2 个子屏。子屏不占导航项。

```text
导航      ③ 诊断工作台      ④ 人工复核    ⑤ 报告归档    ⑥ 知识库
...
│ 记录表格（可点选）      ├─────────────────────┬─────────────────────┤
│ 列由 columns 契约声明  │ 点击记录后打开 AI 判断浮窗                   │
│ 时间 部位 检查项       │  ┌───────────────────────────────────────┐ │
│ 人工结果 AI标记        │  │ AI 辅助判断列表：时序 / 视觉 / 规则      │ │
│                      │  │ [查看告警数据] [查看视觉模型] [复核]    │ │
│ AI标记三态：           │  │ 归档前案例依据锁定，归档后解锁复用       │ │
│  ● conflict 人机冲突   │  └───────────────────────────────────────┘ │
│  ● gap      记录缺项   │                                           │
│  ● ok       人机一致   │  右下角统一 AI 浮动图标 → Agent 对话浮窗    │
└──────────────────────┴───────────────────────────────────────────┘
```
...
│ ① 证据摘要（只读）    │ ② 人工介入区                                 │
│                     │                                             │
│ 3 张关键测点 mini 图  │ ┌ L0 对 AI 结论表决 ────────────────────┐   │
│ + 知识命中 ≤3 条      │ │ AI 建议：{label}（置信度 {n}%）        │   │
│ + 一句案情摘要        │ │  [✓ 采纳]  [✎ 修正]  [✗ 驳回]         │   │
│                     │ └───────────────────────────────────────┘   │
│ 这段固定显示本案例的   │                                             │
│ "证据指纹"，不随你在   │ ┌ L1 结论选择（chips，单选） ────────────┐   │
│ 哪个部位而变          │ │  ○ … ○ … ○ …   （由 outcomes 驱动）  │   │
│                     │ └───────────────────────────────────────┘   │
│                     │                                             │
│                     │ ┌ L2 结构化补充（随结论动态换） ───────────┐  │
│                     │ │ 处置班组[▾] 复测窗口[▾] 风险等级[▾]      │  │
...
│                     │ │ [多行文本框，可打字]                    │   │
│                     │ │ 常用语：[现场已确认][需备件][暂缓]…      │   │
│                     │ │   ← 点 chip 追加进文本框，演示不用打字   │   │
│                     │ └───────────────────────────────────────┘   │
│                     │                                             │
│                     │ ⚠ 分歧提示（人工 ≠ AI 时出现，理由必填）      │
├─────────────────────┴─────────────────────────────────────────────┤
│ ③ 处置路径：未选结论前整段 dimmed 且不可交互，选定后展开步骤 + 执行按钮  │
└───────────────────────────────────────────────────────────────────┘
```
...
| **L1 结论** | chips 单选 | "结论是排他的" | 驱动整个分支状态机 |
| **L2 结构化** | 下拉 + 勾选 | "人的判断有结构，能被系统消费" | 字段值注入报告模板插槽 |
...
3. **二次介入点**。执行屏底部再放一个轻量介入：复测结果 **[通过] / [不通过，退回复核]**。选"不通过"→ `executed` 置回 `false`、退回本页并提示"复测未通过，请重新确认结论"（`outcomeId` 和 `note` 保留，允许改也允许维持）。**demo 因此有一条回头路**，不是一条只能往前走的轨道。

**执行屏**（`review.executed === true` 后的子态，沿用 pump-demo 结构）：`track === "treatment"` → 处置票卡 + 前后对比图 + 复测验收；`track === "closure"` → 结论闭环卡 + 依据摘要 + 闭环动作。
...
- **归档动画是闭环的视觉锚点**。报告卡飞进知识库图标 → 知识库页"归档案例"分类计数 +1、文档列表顶部出现这篇新文档 → 之后 Agent 能命中它。这是把归档和知识库真正缝上的那一针。
...
window.DOMAIN_META = {
  ...
  scenes:    [{ key, label, node }],   // 恰好 4 项，key 固定为 workbench/review/archive/knowledge
