...
window.ReportView = (function () {
    ...
    if (sec.kind === "grid") {
      ...
      if (sec.id === "SEC-SOURCE") {
        ...
        children.push(h("figure", { class: "rp-photo" }, [
          h("img", {
            src: VISION.sourceOf("sitePhotoValve"),
            alt: "巡检人就地核对阀门电动执行机构"
          }),
          h("figcaption", { text: "现场佐证：巡检人持手持终端就地核对阀门电动执行机构与就地压力表。" })
        ]));
      }
  ...
  function modelOfState() {
    ...
    return ReportModel.resolve(ReportModel.contextFromState({
      ...
      note: state.note,
      range: state.range,
      // 生成时间取自数据里那一晚，不用 new Date() —— 演示机跨天之后报告上的时间
      // 不该跟着变（画面上四张关键帧的 OSD 时间是固定的，对不上就露）。
      generatedAt: "2026-07-22 20:47"
    }));
  }
